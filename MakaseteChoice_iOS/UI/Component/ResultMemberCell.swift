...
struct ResultMemberCell: View {
    ...
    private func getMemberPinnedStatus() -> Bool {
        let targetMemberIndex = self.appState.memberObject.members.index(where: { $0.name == member.name })
        return self.appState.memberObject.members[targetMemberIndex!].isPinned
    }
}
